...
This guide provides testing procedures to validate the
`IDP-Cloudformation-Service-Role.yaml` CloudFormation service role for GenAI IDP
...
**IMPORTANT**: The following steps require a user or role with permissions to
deploy IAM roles.
...
**Objective**: Test the CloudFormation service role's ability to deploy and
update IDP stacks.

**Prerequisites**: The iam:PassRole policy created by the role template must be
attached to the user or role performing IDP stack changes.
...
2. **For IAM role, choose IDPAcceleratorCloudFormationServiceRole** from
   dropdown
...
- **Resource Changes**: Pattern 2 nested stack removed, Pattern 1 (BDA) nested
  stack created
...
- **Assumption Success**: Non-administrator user or role can assume the
